<?php
if (isset($_GET['action'])) {
    if ($_GET['action'] == "articuloRegistrado") {
        print '<div class="alert alert-success d-flex align-items-center" role="alert">
        <svg class="bi flex-shrink-0 me-2" width="24" height="24" role="img" aria-label="Success:"><use xlink:href="#check-circle-fill"/></svg>
        <div>
          Articulo agregado
        </div>
      </div>';
    }
    if ($_GET['action'] == "articuloActualizar") {
        print '<div class="alert alert-success d-flex align-items-center" role="alert">
        <svg class="bi flex-shrink-0 me-2" width="24" height="24" role="img" aria-label="Success:"><use xlink:href="#check-circle-fill"/></svg>
        <div>
          Articulo actualizado
        </div>
      </div>';
    }
}
$conn = new ControladorArticulo();
$con = $conn->contarDatosArticuloControlador();
$limtPagina = 10;
$pagina = $con[0]['COUNT(id_articulo)'] / $limtPagina;
$pagina = ceil($pagina);
if (isset($_GET['pagina'])) {
    if (!$_GET['pagina']) {
        echo '<script>window.location="index.php?action=articulo&pagina=1"</script>';
    }
    if ($_GET['pagina'] > $pagina || $_GET['pagina'] <= 0) {
        echo '<script>window.location="index.php?action=articulo&pagina=1"</script>';
    }
}
$lis = new ControladorArticulo();
if (isset($_GET['pagina'])) {
    $res = $lis->listarArticuloControlador($_GET['pagina'], $limtPagina);
}
?>
<h1 style="text-align: center;">Articulo</h1>
<div class="container mt-5">
    <div class="row">
        <div class="col">
            <a href="agregarArticulo" class="btn btn-primary">Agregar</a>
            <form method="post" class="mt-3">
                <input type="text" class="form-control" name="buscar" placeholder="Nombre Articulo y/o codigo Producto">
            </form>
        </div>
    </div>
</div>
<table class="table mt-4">
    <thead>
        <tr>
            <th>#</th>
            <th>Codigo</th>
            <th>Articulo</th>
            <th>Categoria</th>
            <th>Medida</th>
            <th>Precio Unitario</th>
            <th>Precio Publico</th>
            <th>Cantidad</th>
            <th>Proeevedor</th>
        </tr>
    </thead>
    <tbody>
        <?php
        if (isset($_GET['pagina'])) {
            foreach ($res as $key => $value) {
                ?>
                <tr>
                    <td>
                        <?php echo $value['id_articulo'] ?>
                    </td>
                    <td>
                        <?php echo $value['codigo_producto'] ?>
                    </td>
                    <td>
                        <?php echo $value['nombre_producto'] ?>
                    </td>
                    <td>
                        <?php echo $value['categoria'] ?>
                    </td>
                    <td>
                        <?php echo $value['medida'] ?>
                    </td>
                    <td>
                        <?php echo $value['precio_unitario'] ?>
                    </td>
                    <td>
                        <?php echo $value['valor_producto_iva'] ?>
                    </td>
                    <td>
                        <?php echo $value['cantidad_producto'] ?>
                    </td>
                    <td>
                        <?php echo $value['nombre_proeevedor'] ?>
                    </td>
                </tr>
                <?php
            }
        }
        ?>
    </tbody>
</table>
<?php if (!isset($_POST['buscar'])) { ?>
    <div class="row">
        <div class="col-sm-4"></div>
        <div class="col">
            <nav aria-label="Page navigation example">
                <ul class="pagination">
                    <li class="page-item <?php echo $_GET['pagina'] <= 1 ? 'disabled' : '' ?>"><a class="page-link"
                            href="index.php?action=articulo&pagina=<?php echo $_GET['pagina'] - 1 ?>">Anterior</a></li>
                    <?php for ($i = 0; $i < $pagina; $i++): ?>
                        <li class="page-item <?php if ($_GET['pagina'] == $i + 1) {
                            print 'active';
                        } ?>"><a class="page-link" href="index.php?action=articulo&pagina=<?php echo $i + 1 ?>">
                                <?php echo $i + 1 ?>
                            </a></li>
                    <?php endfor ?>
                    <li class="page-item <?php echo $_GET['pagina'] >= $pagina ? 'disabled' : '' ?>"><a class="page-link"
                            href="index.php?action=articulo&pagina=<?php echo $_GET['pagina'] + 1 ?>">Siguiente</a></li>
                </ul>
            </nav>
        </div>
    </div>
<?php } ?>